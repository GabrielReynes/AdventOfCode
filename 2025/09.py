...
def part1(input:str): 
    ...
    return max((abs(ax-bx)+1)*(abs(ay-by)+1) for (ax, ay),(bx, by) in combinations(coords, 2))
...
def part2(input:str): 
    ...
    def is_inside(x:int, y:int) -> int:
        cnt = sum(x1 == x2 and x < x1 and min(y1, y2) <= y < max(y1, y2) 
                  for (x1, y1), (x2, y2) in edges)
        return cnt & 1

    grid = [[is_inside( (x1 + x2) // 2, (y1 + y2) // 2)
