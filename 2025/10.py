...
def gauss_jordan_solve(moves, target):
    ...
    free_var_bounds = [
        min((target[d] // moves[fv][d] for d in range(dim_count) if moves[fv][d] > 0), default=float('inf'))
        for fv in free_vars
    ]
...
def part2(input:str): 
    ...
    for l in lines :
        ...
        print("TARGET", target)
        buttons = sorted((tuple(map(int, b[1:-1].split(','))) for b in s_buttons), 
